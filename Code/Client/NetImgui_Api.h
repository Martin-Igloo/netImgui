...
NETIMGUI_API	bool				IsTexturePendingSend(const ImTextureRef& textureRef);   // True if a previous send for this texture is still pending transmission to the server
// Like SendDataTexture but instructs the server to retain its CPU pixel buffer — required before any SendDataTextureUpdate calls for the same ID.
NETIMGUI_API	void				SendDataTextureCreate(const ImTextureRef& textureRef, void* pData, uint16_t width, uint16_t height, eTexFormat format, uint32_t dataSize=0);
// Patch an existing server texture in-place (no reallocation). Requires a prior SendDataTextureCreate for the same ID.
NETIMGUI_API	void				SendDataTextureUpdate(const ImTextureRef& textureRef, void* pData, uint16_t width, uint16_t height, eTexFormat format, uint32_t dataSize=0);
#endif
